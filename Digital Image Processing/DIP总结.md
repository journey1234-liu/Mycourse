...
2. RGB成像： 通过**拜耳滤镜**或者多传感器，获得的R、G、B通道的图像通过临近像素均值法（双线性插值）或者临近像素复制法进行插值，获得彩色图像。
...
注意m-连接，是为了消除8-连接的歧义性。有4-连接就用4-连接，4-连接没有就看是否有8-连接。
...
灰值腐蚀：如果所有结构元素是正的，输出图像会更暗，比结构元素小的区域，亮细节减弱

灰值膨胀：、、、，输出会更亮，如果暗细节的面积比较小，可能会被消除
...
top-hat高帽变换：检测波峰，**在较亮的背景中求暗的像素聚集体(颗粒)在较暗的背景中求亮的像素聚集体**

bot-hat低帽变换：检测波谷，**在较亮的背景中求暗的像素点**
...
## 第九章 特征提取的深度网络![9深度网络架构](fig\9深度网络架构.png)
...
特征图展开为1维向量，从flatten层输入到全连接层（减少特征位置对分类带来的影响）
...
1. inception结构，使深度达到了22层。稀疏性，高计算性能的结构。
