...
- 采样对频域的影响——时域离散化导致频域周期化
  ...
  如果我们将经过采样后的信号仍旧是做连续信号，则：
  $$
  X_{s}(j\Omega)=F_{s}\sum_{k=-\infty}^{\infty}X_{c}\left[j\left(\Omega-k\frac{2\pi}{T_{s}}\right)\right]
  $$

也就是说，时域上的采样，导致频域上的频谱发生了幅值扩张以及周期化，对应的频率周期为$F_{s}$，角频率的周期是$\Omega_{s}$。

### 采样定理

对于一个频率有限的信号，如果想要保证采样重建之后的信号不发生混叠，采样频率$F_{s}\geq 2F_{h}$。
